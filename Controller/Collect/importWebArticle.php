...
class importWebArticle extends Action
{
        ...
        foreach ($list as $k => $v) {
            ...
            if (!empty($data)) {
                if (!empty($data['Content'])) {
                    ...
                    $mxtag = Article::getStarTag($content);
                    if (!empty($mxtag)) {
                        $data['TagCache'] = !empty($data['TagCache']) ? array_merge($data['TagCache'], $mxtag) : $mxtag;
                        $data['ViewTag'] = !empty($data['ViewTag']) ? array_merge($data['ViewTag'], $mxtag) : $mxtag;
                    }
                }
                ...
                if (count($data['ViewTag']) > 5) {
                    $data['ViewTag'] = array_slice($data['ViewTag'], 0, 5);
                }
                if (!empty($data['UpdateTime']) && strtotime($data['UpdateTime']) > time()) {
    ...
    private static function buildData($info)
    {
        ...
        if (!empty($info) && self::$isadd) {
            ...
            $sites = loadconf('article/site');
            if (!empty($sites[self::$siteid2]['name'])) {
                $tags[] = $sites[self::$siteid2]['name'];
            }
            ...
            $data = [
                ...
                'SiteId' => self::$siteid2,
                'SiteName' => !empty(self::$sitename) ? self::$sitename : '',
                'Author' => $info['Author'],
