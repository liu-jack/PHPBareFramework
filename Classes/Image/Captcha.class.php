...
class Captcha
{
    ...
    public static function entry()
    {
        ...
        for ($i = 0; $i < self::$length; $i++) {
            ...
            // 写一个验证码字符
            imagettftext(self::$_image, self::$fontSize, mt_rand(-40, 70), $codeNX, self::$fontSize * 1.5,
                self::$_color, $ttf, $code[$i]);
        }
    ...
    protected static function _writeCurve()
    {
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 0.9) {
            if ($w != 0) {
                ...
                while ($i > 0) {
                    imagesetpixel(self::$_image, $px + $i, $py + $i,
                        self::$_color);  // 这里画像素点比imagettftext和imagestring性能要好很多
                    $i--;
                }
            }
        }
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 0.9) {
            if ($w != 0) {
                ...
                while ($i > 0) {
                    imagesetpixel(self::$_image, $px + $i, $py + $i,
                        self::$_color);  // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
